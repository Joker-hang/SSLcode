...
def get_config():
    ...
    '''
    Saving & loading of the model.保存和加载模型
    '''
    ...
    '''
    Training Configuration of FixMatch 训练配置
    '''
    ...
    parser.add_argument('-nl', '--num_labels', type=int, default=400)  # 有标记图片数量
    parser.add_argument('-tl', '--train_lb', type=float, default=0.1)  # 有标记图片占比
    ...
    '''
    Optimizer configurations 优化器配置
    '''
    ...
    '''
    Backbone Net Configurations 骨干网络
    '''
    ...
    '''
    Algorithms Configurations 算法配置
    '''  
    ...
    # parser.add_argument('--crop_ratio', type=float, default=0.875) #裁剪区域占比
    parser.add_argument('--crop_ratio', type=int, default=1)  # 裁剪padding
    ...
    '''
    multi-GPUs & Distrbitued Training 分布式训练配置
    '''
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # random seed has to be set for the syncronization of labeled data sampling in each process. 在每个过程中，为了保证标记数据采样的同步，需要设置随机种子。
